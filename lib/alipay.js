...
Alipay.prototype.refund = function (data) {
  var _refund = {
    "out_trade_no":data.trade_id,
    "refund_amount":data.actual_fee,
    "out_request_no" : data.out_request_no || data.trade_id
  };
  var params = {
    app_id: this.config.app_id,
  ...
  var biz_conent = _refund;

  params.biz_content = JSON.stringify(biz_conent);

  params = this.sign(params, false);
  debug('refund data is ready: %j', params);
  ...
  return request(options).then(
    res=>{
    debug("alipay refund trade , return data ",res)
    var response = JSON.parse(res)
    return Promise.resolve(response);
  })
...
Alipay.prototype.transfer = function (data) {
  ...
  params.biz_content = JSON.stringify(data);
  ...
  var options = {
    method: 'POST',
    uri: "https://openapi.alipay.com/gateway.do?charset=utf-8",
    form: common.queryize(params)
  }
  return request(options).then(
    res=>{
    debug("alipay transfer trade , return data ",res)
    var response = JSON.parse(res)
    return Promise.resolve(response);
  })
